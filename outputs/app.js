...
const gaodeGeocodingUrl = "https://restapi.amap.com/v3/geocode/geo";
...
const gaodeApiKey =
  globalThis.GAODE_API_KEY ||
  globalThis.AMAP_API_KEY ||
...
async function searchPlaces(query) {
  ...
  if (gaodeApiKey) {
    const gaodeResults = await searchPlacesGaode(query);
    if (gaodeResults.length > 0) {
      return gaodeResults;
    }
  }
...
async function searchPlacesGaode(query) {
  try {
    const url = buildUrl(gaodeGeocodingUrl, {
      address: query,
      key: gaodeApiKey,
      city: "",
      output: "JSON"
    ...
    return data.geocodes.map((geocode) => {
      ...
      return {
        ...
        admin2: geocode.city || "",
        admin1: geocode.province || "",
        country: "中国",
        source: "gaode",
        displayName: geocode.formatted_address || geocode.district || query
      };
    });
  } catch {
    return [];
